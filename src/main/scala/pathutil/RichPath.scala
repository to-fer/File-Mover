...
class RichPath(delegatePath: Path) {
  ...
  def extension = {
    val name = delegatePath.getFileName.toString
  ...
  def downloadFinish(timeout: Duration = Duration.create(15, TimeUnit.MINUTES)): Future[Path] = future {
    ...
    val blockTime = Duration.create(50, TimeUnit.MILLISECONDS)
    ...
    def blockTillDownloadFinish(prevDigest: List[Byte], timeLeft: Duration): Unit = {
       ...
       */
        Thread.sleep(blockTime.length) // DO NOT REMOVE THIS. I'M WARNING YOU!
         ...
         */
        if (!(curDigest sameElements prevDigest) || Files.size(delegatePath) == 0) blockTillDownloadFinish(curDigest, timeout - blockTime)
      }
